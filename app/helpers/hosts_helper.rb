module HostsHelper
  ...
  def multiple_actions
    ...
    if authorized_for(:controller => :hosts, :action => :edit)
      ...
      actions <<  [_('Change Puppet Master SmartProxyPool'), select_multiple_puppet_proxy_pool_hosts_path] if SmartProxy.unscoped.authorized.with_features("Puppet").exists?
      actions <<  [_('Change Puppet CA SmartProxyPool'), select_multiple_puppet_ca_proxy_pool_hosts_path] if SmartProxy.unscoped.authorized.with_features("Puppet CA").exists?
    end
  ...
  def multiple_proxy_pool_select(form, proxy_feature)
    selectable_f form,
      :pool_id,
      [[_("Select desired %s Proxy Pool") % _(proxy_feature), "disabled"]] +
      [[_("*Clear %s Proxy Pool*") % _(proxy_feature), ""]] +
      Feature.find_by_name('Puppet').smart_proxy_pools.map {|h| [h.name, h.id]},
      {},

class SmartProxy < ActiveRecord::Base
  ...
  has_many :urls, :class_name => 'SmartProxyUrl'
  ...
  before_save :associate_features

  scoped_search :on => :name, :complete_value => :true
  scoped_search :relation => :features, :on => :name, :rename => :feature, :complete_value => :true
  scoped_search :relation => :urls, :on => :url, :rename => :url, :complete_value => :true
  ...
  def self.smart_proxy_ids_for(hosts)
    ...
    ids << hosts.map { |host| host.puppet_proxy.id}
    ids << hosts.map { |host| host.puppet_ca_proxy.id}
    ids << hosts.map { |host| host.hostgroup.try(:'puppet_proxy.id')}
    ids << hosts.map { |host| host.hostgroup.try(:'puppet_ca_proxy.id')}
  ...
  def taxonomy_foreign_conditions
    ...
    if has_feature?('Puppet') && has_feature?('Puppet CA')
      conditions = "puppet_proxy_url_id = #{id} OR puppet_ca_proxy_url_id = #{id}"
    elsif has_feature?('Puppet')
      conditions[:puppet_proxy_url_id] = id
    elsif has_feature?('Puppet CA')
      conditions[:puppet_ca_proxy_url_id] = id
    end
    conditions
  end
  ...
  def url
    Foreman::Deprecation.deprecation_warning('1.17', "Use primary_url instead of url for accessing a Smary Proxy url attribute")
    SmartProxy.reset_column_information
    if SmartProxy.column_names.include? 'url'
      read_attribute(:url)
    else
      self.primary_url
    end
  ...
  def primary_url
    self.urls.map { |u| if u.primary = true then u.url end }.first
  end

  def secondary_urls
   self.urls.where(:primary => false)
  end
  ...
  def associate_features
    begin
      reply = ProxyAPI::Features.new(:url => primary_url).features

class SmartProxy < ApplicationRecord
  ...
  has_many_hosts :through => 'hostnames'
  has_many :hostgroups, :through => 'hostnames'
  has_many :puppet_ca_hosts, :class_name => 'Host::Managed', :through => 'hostnames'
  has_many :puppet_ca_hostgroups, :class_name => 'Hostgroup', :through => 'hostnames'
  ...
  before_create :create_hostname

  scoped_search :on => :name, :complete_value => :true
  scoped_search :on => :url, :complete_value => :true
  scoped_search :relation => :hostnames, :on => :name, :rename => :hostname, :complete_value => :true
  ...
  def self.smart_proxy_ids_for(hosts)
    ...
    ids << hosts.map { |host| host.puppet_proxy.try(:id)}
    ids << hosts.map { |host| host.puppet_ca_proxy.try(:id)}
  ...
  def create_hostname
    self.hostnames << SmartProxyPool.create_with(name: self.name).find_or_initialize_by(hostname: self.hostname)
  end

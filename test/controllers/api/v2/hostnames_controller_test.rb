...
class Api::V2::SmartProxyPoolsControllerTest < ActionController::TestCase
  ...
  test "should create valid hostname" do
    post :create, params: { :hostname => { :name => "hostname.net", :hostname => 'hostname.net' } }
  ...
  test "should not create invalid hostname" do
    post :create, params: { :hostname => { :name => "" } }
    assert_response :unprocessable_entity
  end

  test "should update valid hostname" do
    put :update, params: { :id => SmartProxyPool.unscoped.first.to_param, :hostname => { :name => "hostname.new" } }
    assert_equal "hostname.new", SmartProxyPool.unscoped.first.name
    assert_response :success
  end

  test "should not update invalid hostname" do
    put :update, params: { :id => SmartProxyPool.first.to_param, :hostname => { :name => "" } }
    assert_response :unprocessable_entity
  end
  ...
  test "should destroy hostname" do
    ...
    delete :destroy, params: { :id => hostname.to_param }
    hostname = ActiveSupport::JSON.decode(@response.body)
    assert_response :ok
    refute SmartProxyPool.find_by_id(hostname['id'])
  end
  ...
  test "should show hostname with correct child nodes including location and organization" do
    get :show, params: { :id => hostnames(:one).to_param }
    assert_response :success
    show_response = ActiveSupport::JSON.decode(@response.body)
    assert !show_response.empty?

...
class HostJSTest < IntegrationTestWithJavascript
  ...
  describe 'edit page' do
    ...
    test 'choosing a hostgroup does not override other host attributes' do
      original_hostgroup = FactoryBot.
        create(:hostgroup, :environment => FactoryBot.create(:environment),
                           :puppet_proxy_hostname => FactoryBot.create(:puppet_smart_proxy).hostnames.first)
      ...
      assert_equal original_hostgroup.puppet_proxy_hostname.name, find("#s2id_host_puppet_proxy_hostname_id .select2-chosen").text

      click_on_inherit('puppet_proxy_hostname')
      select2(overridden_hostgroup.name, :from => 'host_hostgroup_id')
      wait_for_ajax
      ...
      assert_equal '', find("#s2id_host_puppet_proxy_hostname_id .select2-chosen").text
    end

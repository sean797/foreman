...
class HostTest < ActiveSupport::TestCase
  ...
  context "location or organizations are not enabled" do
    ...
    test "handle_ca must not perform actions when no Puppet CA proxy is associated even if associated with hostgroup" do
      ...
      h.puppet_proxy_pool_id = h.puppet_ca_proxy_pool_id = nil
      h.save
    ...
    test "can search hosts by smart proxy pool" do
      ...
      results = Host.search_for("puppetmaster_pool = #{pool.name}")
      assert_equal 0, results.count
      results = Host.search_for("puppet_ca_pool = #{pool.name}")
      assert_equal 0, results.count
      host.update_attribute(:puppet_proxy_pool_id, pool.id)
      host.update_attribute(:puppet_ca_proxy_pool_id, pool.id)

class ProxiesHavePools < ActiveRecord::Migration[5.0]
  def up
    ...
    add_column :hostgroups, :puppet_ca_proxy_pool_id, :integer
    add_column :hostgroups, :puppet_proxy_pool_id, :integer
  ...
  def down
    ...
    remove_column :hosts, :puppet_ca_proxy_pool_id, :integer
    remove_column :hosts, :puppet_proxy_pool_id, :integer
    ...
    Hostgroup.unscoped.each do |group|
      if group.puppet_ca_proxy_pool_id
        group.puppet_ca_proxy_id = SmartProxy.unscoped.joins(:smart_proxy_pools).where(smart_proxy_pools: { id: group.puppet_ca_proxy_pool_id }).try(:first).try(:id)
      end
      if group.puppet_proxy_pool_id
        group.puppet_proxy_id = SmartProxy.unscoped.joins(:smart_proxy_pools).where(smart_proxy_pools: { id: group.puppet_proxy_pool_id }).try(:first).try(:id)
      end
      group.save!(:validate => false)
    end
    remove_column :hostgroups, :puppet_ca_proxy_pool_id, :integer
    remove_column :hostgroups, :puppet_proxy_pool_id, :integer
